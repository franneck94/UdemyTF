...
def dense_block(
    ...
    x1 = Conv2D(filters=filters, strides=1, kernel_size=3, padding="same")(x)
    x1 = relu_norm(x1)
    x2 = Conv2D(filters=filters, strides=1, kernel_size=3, padding="same")(x1)
    x2 = relu_norm(x2)
    out = Concatenate()([x1, x2])
    if bottleneck:
        out = Conv2D(filters=filters, strides=1, kernel_size=1, padding="same")(
            out
        )
    if downsample:
...
def build_model_densenet(
    ...
    x = dense_block(x=input_img, filters=32, downsample=True, bottleneck=True)
    x = dense_block(x=x, filters=64, downsample=False, bottleneck=True)
    x = dense_block(x=x, filters=64, downsample=False, bottleneck=True)
    x = dense_block(x=x, filters=128, downsample=True, bottleneck=True)
    x = dense_block(x=x, filters=128, downsample=False, bottleneck=True)
...
if __name__ == "__main__":
    ...
    model = build_model_densenet(img_shape, num_classes)
